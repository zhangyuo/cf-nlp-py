...
class summary(object):
    ...
    def pkusumsum(self, input, output, T=1, L=1, n=500, m=2, stop='n', topic=''):
        """
        ...
        :param topic: the path of the topic file only for the topic-based multi-document summarization
        :return:
        """
        if topic != '':
            command = "java -jar %s -T %d -input %s -output %s -L %d -n %d -m %d -stop %s -topic %s" % (
                self.jar_path, T, input, output, L, n, m, stop, topic)
        else:
    ...
    def single_text_abstract(self, instring, L=1, n=500, m=2, stop='n', topic=''):
        """
        单文本摘要
        :param instring: 输入文本
        :param L: 1-中文；2-英文；3-其他语言
...
if __name__ == '__main__':
    ...
    # topic-based multiple docs summary
    # model.pkusumsum('/Users/zhangyu/Documents/data/data_test/abstract/multiple/',
    #                 '/Users/zhangyu/Documents/data/data_test/abstract/result.txt', T=3, m=2, n=100,
    #                 topic='/Users/zhangyu/Documents/data/data_test/abstract/topic.txt')
